...
LAB_NAME = Path(__file__).resolve().parent.name
LAB_DIR = REPO_ROOT / "src" / LAB_NAME


def solution_path(task_num: int) -> Path:
    candidate = LAB_DIR / f"task{task_num}.py"
    if candidate.exists():
        return candidate
    alt = LAB_DIR / f"task{task_num}" / f"task{task_num}.py"
    if alt.exists():
        return alt
    raise FileNotFoundError(
        f"Не найден файл решения для task{task_num}. "
        f"Ожидался: {candidate}"
    )
...
def _measure_baseline_bytes() -> int:
    """
    Один раз запускает пустой скрипт через тот же wrapper
    и запоминает пиковую память процесса.
    Это «пол» — цена запуска интерпретатора + импортов.
    """
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        _BASELINE_BYTES = int(mem_file.read_text().strip()) if mem_file.exists() else 0

    return _BASELINE_BYTES


def run_task(task_path, input_data: str, timeout: float = 30):
    """
    Запускает задачу в изолированной временной папке.

    :return: (output_text, elapsed_seconds, peak_memory_bytes)
    """
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        mem_file = tmp / "mem.txt"
        mem_bytes = int(mem_file.read_text().strip()) if mem_file.exists() else 0
...
def check(task_num: int, input_data: str, expected: str | None = None,
          timeout: float = 30) -> str:
    output, elapsed, mem_bytes = run_task(
        solution_path(task_num), input_data, timeout=timeout
    )

    total = mem_bytes + _measure_baseline_bytes()
    print(f"\n[task{task_num}] time={elapsed:.3f}s  "
